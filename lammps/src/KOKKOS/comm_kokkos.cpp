...
{
  if (fix->execution_space == Host || !fix->reverse_comm_device || reverse_comm_classic ||
...
void CommKokkos::reverse_comm_device(Fix *fix, int size)
{
  ...
  for (iswap = nswap-1; iswap >= 0; iswap--) {
    ...
    // pack buffer from ghost atoms

    n = fixKKBase->pack_reverse_comm_kokkos(recvnum[iswap],firstrecv[iswap],
                                      k_buf_send_fix);
    DeviceType().fence();
    ...
    // unpack buffer into owned atoms
