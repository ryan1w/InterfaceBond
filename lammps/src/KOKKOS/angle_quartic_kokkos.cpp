  ...
  if (evflag) {
    if (newton_bond)
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType,
                            TagAngleQuarticCompute<1,1> >(0, nanglelist), *this, ev);
    else
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType,
                            TagAngleQuarticCompute<0,1> >(0, nanglelist), *this, ev);
  } else {
    if (newton_bond)
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType,
                          TagAngleQuarticCompute<1,0> >(0, nanglelist), *this);
    else
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType,
                          TagAngleQuarticCompute<0,0> >(0, nanglelist), *this);
  }
...
void AngleQuarticKokkos<DeviceType>::operator()(TagAngleQuarticCompute<NEWTON_BOND, EVFLAG>,
                                                const int &n, EV_FLOAT &ev) const
{
  ...
  const F_FLOAT tk = 2.0*d_k2[type]*dtheta + 3.0*d_k3[type]*dtheta2 +
    4.0*d_k4[type]*dtheta3;

  F_FLOAT eangle = 0.0;
  if (eflag) eangle = d_k2[type]*dtheta2 + d_k3[type]*dtheta3 + d_k4[type]*dtheta3*dtheta;

  const F_FLOAT a = -tk * s;
...
{
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND, EVFLAG>(TagAngleQuarticCompute<NEWTON_BOND, EVFLAG>(),
                                                 n, ev);
}
...
template<class DeviceType>
void AngleQuarticKokkos<DeviceType>::allocate_kokkos()
{
  int n = atom->nangletypes;
  k_k2 = typename AT::tdual_ffloat_1d("AngleQuartic::k2", n+1);
  k_k3 = typename AT::tdual_ffloat_1d("AngleQuartic::k3", n+1);
  k_k4 = typename AT::tdual_ffloat_1d("AngleQuartic::k4", n+1);
  k_theta0 = typename AT::tdual_ffloat_1d("AngleQuartic::theta0", n+1);
...
template<class DeviceType>
void AngleQuarticKokkos<DeviceType>::sync_coeffs_to_kokkos()
{
  int n = atom->nangletypes;
  if ((int) k_k2.extent(0) < n+1) allocate_kokkos();
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void AngleQuarticKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int i, const int j,
                                              const int k, F_FLOAT &eangle, F_FLOAT *f1,
                                              F_FLOAT *f3, const F_FLOAT &delx1,
                                              const F_FLOAT &dely1, const F_FLOAT &delz1,
                                              const F_FLOAT &delx2, const F_FLOAT &dely2,
                                              const F_FLOAT &delz2) const
{
  E_FLOAT eanglethird;
  F_FLOAT v[6];
  ...
  Kokkos::View<E_FLOAT*, typename DAT::t_efloat_1d::array_layout,
               typename KKDevice<DeviceType>::value,
               Kokkos::MemoryTraits<Kokkos::Atomic | Kokkos::Unmanaged> > v_eatom =
    k_eatom.template view<DeviceType>();
  Kokkos::View<F_FLOAT*[6], typename DAT::t_virial_array::array_layout,
               typename KKDevice<DeviceType>::value,
               Kokkos::MemoryTraits<Kokkos::Atomic | Kokkos::Unmanaged> > v_vatom =
    k_vatom.template view<DeviceType>();
